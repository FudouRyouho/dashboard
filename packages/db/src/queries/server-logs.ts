...
export async function insertServerLog(
  db: DB,
  input: InsertServerLogInput,
): Promise<ServerLogEntryRow | undefined> {
  const timestamp = new Date();
  const result = await db
    .insert(serverLogEntries)
    .values({
      timestamp,
      level: input.level,
...
export async function purgeServerLogsOlderThan(
  db: DB,
  cutoff: Date,
): Promise<{ deleted: number }> {
  const result = await db
    .delete(serverLogEntries)
    .where(lt(serverLogEntries.timestamp, cutoff));
