...
export function CalenderDay({ date, events, disabled }: CalendarDayProps) {
    ...
    if (!isWeekend && !disabled) textColor = "white"; // Día semana, con evento
    else if (isWeekend && !disabled) textColor = "lime"; // Finde, con evento
    ...
    return (
        ...
        >
            {notificationEvents.map((notificationEvent) => {
                return <Box key={notificationEvent} bg={notificationEvent} h={8} w={"100%"} p={0} style={{ borderRadius: 999 }} />;
            })}
